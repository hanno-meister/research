...
Synthesis discipline: when discussing each named system, technology, or category, explicitly address what happens after the primary capability — for a generative system, can outputs be acted on, edited, navigated, exported, or simulated, and with what supporting evidence; for an infrastructure system, what does it enable downstream and with what supporting evidence; for a benchmark or evaluation system, what does it measure and what does it leave unmeasured. Do not describe systems in isolation when the bundle contains adjacent evidence about their downstream use, interaction patterns, or integration paths. Surface those connections in the same paragraph rather than relegating them to footnotes or separate sections.
Cross-task coherence: identify the most important unresolved tension or open question that emerges across multiple findings, and use it as a framing anchor in Deep Dive and Why It Matters rather than letting each section restate the same evidence independently. Findings from different tasks should build on each other where the evidence supports it.
...
Deep Dive should be a coherent multi-paragraph narrative overview of the most important theme/technology, explaining what the sources collectively say, where they agree, where evidence is weaker, and what that means for the Lance context. Do not make Deep Dive a list of disconnected finding summaries.
...
Confidence and Gaps discipline: for each material coverage gap, state one specific recommendation, decision, or strategic question that cannot be made with confidence until that gap is closed. Do not list gaps as standalone caveats; tie each one to what it blocks. If a gap blocks nothing actionable, omit it from this section rather than including it for completeness.
