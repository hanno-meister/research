...
- `src/vanguard/research/search_gateway.py` is standalone Exa/Tavily provider plumbing; tests inject fake clients and must not call external search APIs.
...
- No repo-specific lint, typecheck, formatter, API run script, CI, or pre-commit config is present yet.
...
- `plan_research` uses the large model with structured `ResearchPlan`; sanitized `focused_domains` must stay within `allowed_domains`, and empty/invalid plans raise `ValueError`.
...
- Additive `AgentState` fields (`research_findings`, `research_sources`, `evidence_artifacts`, `research_feasibility_notes`, `source_diversity_notes`, `research_reviews`, `evidence_read_records`) must be returned as lists to append, not scalars; `research_tasks` is not additive.
